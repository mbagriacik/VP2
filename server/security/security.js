...
app.post(properties.api + '/login', function(req, res, md5pwd) {
    const db_Vitapio_db = require('../db/Vitapio_db_schema.js');
...
app.post(properties.api + '/changePassword', function(req, res) {
    ...
    db_Vitapio_db.User.findOne({ username: req.user.username, password: req.body.passwordOld }).exec(function(err, user) {
...
var getUser = function(username, password, callback) {
    ...
    db_Vitapio_db.User.findOne({ username: username, password: password }).lean().exec(function(err, user) {
        if (user)
            user.password = undefined;
        callback(user);
    });
...
var createUser = function() {
    ...
    db_Vitapio_db = require('../db/Vitapio_db_schema.js');
    db_Vitapio_db.User.count().exec(function(err, count) {
        if (count == 0) {
            var admin = new db_Vitapio_db.User({
                username: 'admin',
